...
export async function createCard(
    ...
    // Process values and capabilities as strings
    const values =
        typeof card.values === "string"
            ? card.values.split(",").map((v) => v.trim())
            : [];
    const capabilities =
        typeof card.capabilities === "string"
            ? card.capabilities.split(",").map((c) => c.trim())
            : [];
    const goals =
    ...
    try {
        ...
        for (const value of values) {
            const valueData = await createValue(value);
            if (valueData) {
                await Promise.all([
                    setField(
                        `${nodes.cards}/${cardId}/values`,
                        valueData.value_id,
                        true,
                    ),
                    setField(
                        `${nodes.values}/${valueData.value_id}/cards`,
                        cardId,
                        true,
                    ),
                ]);
                console.log(
                    `[createCard] Set value edge: ${valueData.value_id} for ${cardId}`,
                );
            }
        }
        ...
        for (const capability of capabilities) {
            const capData = await createCapability(capability);
            if (capData) {
                await Promise.all([
                    setField(
                        `${nodes.cards}/${cardId}/capabilities`,
                        capData.capability_id,
                        true,
                    ),
                    setField(
                        `${nodes.capabilities}/${capData.capability_id}/cards`,
                        cardId,
                        true,
                    ),
                ]);
                console.log(
                    `[createCard] Set capability edge: ${capData.capability_id} for ${cardId}`,
                );
            }
        }
...
// Initialize bidirectional relationships for existing cards and decks
export async function initializeBidirectionalRelationships(): Promise<{
    ...
    try {
        ...
        for (const deck of decks) {
            ...
            const cardIds =
                typeof deck.cards === "object"
                    ...
                    : [];
            console.log(
                `[initializeBidirectionalRelationships] Processing ${cardIds.length} cards for ${deckId}`,
            );
        ...
        console.log(
            `[initializeBidirectionalRelationships] Processed ${processedCount} relationships`,
        );
        return { success: processedCount > 0, processed: processedCount };
    } catch (error) {
...
// Import multiple cards to a deck
...
export { initializeBidirectionalRelationships }; // Explicit export
