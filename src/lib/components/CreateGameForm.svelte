<script lang="ts">
  ...
  // Use properly typed Svelte 5 RUNES state
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (game) {
        log(`Game created successfully: ${game.game_id}`);
        dispatch('statusUpdate', { status: 'success' });
        dispatch('created', { gameId: game.game_id });
      } else {
        log('Game creation returned null');
      ...
      <div class="space-y-4 mb-4">
        ...
        {:else}
          <div class="alert variant-filled-warning mt-2">
            <p>Custom deck creation is coming soon! Please select a predefined deck for now.</p>
          </div>
        {/if}
      </div>
