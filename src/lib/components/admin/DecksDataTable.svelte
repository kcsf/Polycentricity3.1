<script lang="ts">
  ...
  const { refreshTrigger = 0 } = $props<{ refreshTrigger?: number }>();
  ...
  (async () => {
    ...
    for (const { id } of decks) {
      try {
        ...
        newCardCounts[id] = items.length;
      } catch {
        newCardCounts[id] = 0;
      }
    }
    cardCounts = newCardCounts;
  })(); // IIFE pattern for async in effects [doc_2][doc_3]
});
  ...
  function openEditModal(deck: Deck) {
    ...
    if (!deck.creator_ref) {
      console.log('Converting deck to schema format');
      selectedDeck = {
        ...deck,
        creator_ref: '',
