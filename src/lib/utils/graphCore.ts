...
import { AgreementStatus } from '$lib/types'; // Import the enum
...
export function createLinks(
  ...
  agreements.forEach((agreement) => {
    ...
    // benefit link
    links.push({ source: cardIds[0], target: agreementId, type: 'benefit', id: `${cardIds[0]}_to_${agreementId}_benefit`, angle: 0 });
    // obligation links
...
export function setupInteractions(
  ...
  // disable zoom on node elements to allow clicks
  nodeGroup.selectAll<SVGGElement, D3Node>('.node').on('mousedown.zoom', null);
  ...
  // Apply drag behavior to all nodes - keep this on the entire node for better drag experience
  nodeGroup.selectAll<SVGGElement, D3Node>('.node')
    .call(dragBehavior);
    
  // But apply click handler only to the inner circles, not the entire node or wedges
  nodeGroup.selectAll<SVGCircleElement, D3Node>('.node > circle')
    .on('click', (event, d) => {
...
export function updateForces(
  ...
  // Custom force to align nodes based on angles
  const angleForce = () => {
    links.forEach((link) => {
      ...
      // Determine which node is the agreement node
      ...
      // Calculate desired position based on angle
      const desiredDistance = 100; // Match forceLink distance
      const angle = link.angle;
      const dx = Math.cos(angle) * desiredDistance;
      const dy = Math.sin(angle) * desiredDistance;

      const desiredX = agreementNode.x! + dx;
      const desiredY = agreementNode.y! + dy;

      // Apply a force towards the desired position
      const strength = 0.05; // Adjust strength to control how strongly nodes are pulled
      cardNode.vx = (cardNode.vx || 0) + (desiredX - cardNode.x!) * strength;
      cardNode.vy = (cardNode.vy || 0) + (desiredY - cardNode.y!) * strength;
    });
  };

  simulation
    .nodes(nodes)
    .force('link', d3.forceLink<D3Node, D3Link>(links).id((d) => d.id).distance(100).strength(1))
    .force('charge', d3.forceManyBody().strength(-200))
    .force('center', d3.forceCenter(width / 2, height / 2))
    .force('collide', d3.forceCollide<D3Node>().radius((d) => d.type === 'actor' ? 50 : 20).strength(1))
    .force('angle', angleForce) // Add custom angle force
...
export function initializeD3Graph(
  ...
  try {
    ...
    const actorCardMap =
      externalActorCardMap && externalActorCardMap.size > 0
    ...
    nodeElements.append('circle')
      .attr('r', (d) => d.type === 'actor' ? (d.id === activeCardId ? 45 : 30) : 12)
      .attr('fill', (d) => d.type === 'actor' ? '#fff' : 'var(--color-surface-800-200)')
      .attr('stroke', (d) => d.type === 'actor'
        ? (d.id === activeCardId ? 'var(--color-blue-600-400)' : '#e5e5e5')
        : 'var(--color-surface-900-100)')
      .attr('stroke-width', (d) => d.type === 'actor' ? (d.id === activeCardId ? 2 : 1) : 1);

    // Add colored ring for agreement nodes based on status
    nodeElements.filter((d) => d.type === 'agreement')
      .append('circle')
      .attr('r', 14) // Slightly larger than the main circle (12px) to form a ring
      .attr('fill', 'none')
      .attr('stroke', (d) => {
    ...
    const fx = d3.forceX<D3Node>((d) => d.x0!).strength(0);
    const fy = d3.forceY<D3Node>((d) => d.y0!).strength(0);
    const simulation = d3.forceSimulation<D3Node>()
      ...
      .on('tick', () => {
        linkElements
          .attr('x1', (d) => (typeof d.source === 'string' ? nodes.find((n) => n.id === d.source) : d.source)!.x!)
          .attr('y1', (d) => (typeof d.source === 'string' ? nodes.find((n) => n.id === d.source) : d.source)!.y!)
          .attr('x2', (d) => (typeof d.target === 'string' ? nodes.find((n) => n.id === d.target) : d.target)!.x!)
          .attr('y2', (d) => (typeof d.target === 'string' ? nodes.find((n) => n.id === d.target) : d.target)!.y!);
    ...
    // Update forces to apply the angle alignment
