...
export function addDonutRings(
  ...
  cardNodes.each(function (nodeData) {
    ...
    if (
      (!nodeData._valueNames || nodeData._valueNames.length === 0) &&
    ...
    const categories = [
      ...
      },
      {
        ...
        color: "#9BC23D",
        items:
          nodeData.type === "actor" && (nodeData.data as Card).goals
            ? ((nodeData.data as Card).goals as string)
                .split(/[;,.]+/)
                .map((s: string) => s.trim())
                .filter(Boolean)
            : [],
      ...
      },
      {
        ...
        items:
          nodeData.type === "actor" &&
          (nodeData.data as Card).intellectual_property
            ? ((nodeData.data as Card).intellectual_property as string)
                .split(/[;,.]+/)
                .map((s: string) => s.trim())
                .filter(Boolean)
            : [],
      },
      {
        ...
        items:
          nodeData.type === "actor" &&
          (nodeData.data as Card).rivalrous_resources
            ? ((nodeData.data as Card).rivalrous_resources as string)
                .split(/[;,.]+/)
                .map((s: string) => s.trim())
                .filter(Boolean)
            : [],
      ...
      },
      {
        ...
        color: "#6BA96D",
        items:
          nodeData.type === "agreement" &&
          Array.isArray(nodeData.data.obligations)
            ? nodeData.data.obligations
                .map((obligation: ObligationItem) => obligation.text || "")
                .filter(Boolean)
            : [],
      },
    ];
    ...
    categories.forEach((category) => {
      ...
      const proportion =
        totalItems > 0 ? category.items.length / totalItems : 0.25; // Fallback to equal parts if no items
    ...
    mainWedges.forEach(
      ({ category, categoryIndex, startAngle, endAngle, arc }) => {
        ...
        const categoryLabelGroup = categoryGroup
          .append("g")
        ...
        // FIX #2: Make labels visible by default for debugging
        // Temporarily remove opacity and visibility settings to confirm rendering
        const labelContainer = categoryGroup
          .append("g")
          .attr("class", "label-container")
          .attr("pointer-events", "none");
          
        // Debug log to confirm container creation
        console.log(`Created label container for category: ${category.name}`);
        ...
        const overlayGroup =
          node.select(".overlay-group") ||
          node.append("g").attr("class", "overlay-group");

        // FIX #2: Add debug logging to confirm sub-wedges are being created
        // FIX #2: Make sub-wedges visible by default for debugging
        // Temporarily remove opacity and visibility settings to confirm rendering
        const subWedgesContainer = overlayGroup
          .append("g")
          .attr("class", `sub-wedges-${category.name}`)
          .attr("pointer-events", "none");
        
        // Debug log to confirm container creation
        console.log(`Created sub-wedges container for category: ${category.name}`);
        ...
        if (category.items && category.items.length > 0) {
          ...
          if (category.debug) {
            console.log(
              `Category ${category.name}: startAngle=${startAngle.toFixed(4)}, endAngle=${endAngle.toFixed(4)}, itemCount=${itemCount}, anglePerItem=${anglePerItem.toFixed(4)}`,
            );
          ...
          category.items.forEach((item: any, itemIndex: number) => {
            ...
            const labelDistance =
              DIMENSIONS.subWedgeRadius * (1 + gapPercentage);
            ...
            // FIX #1: Update isLeftHalf to exclude 90 degrees to treat it as right side per requirement
            ...
            if (category.name === "rivalrousResources" || category.debug) {
              console.log(
                `Label position: angle=${angleDeg.toFixed(1)}°, isLeftHalf=${isLeftHalf}, x=${labelX.toFixed(1)}, y=${labelY.toFixed(1)}`,
              );
            }
            ...
            if (category.debug) {
              console.log(
                `Rivalrous Resources label angle: ${angleDeg} degrees`,
              );
            }
            ...
            // FIX #1: Update isLeftSide to match isLeftHalf - exclude 90 degrees to treat it as right side
            ...
            if (category.debug || category.name === "rivalrousResources") {
              console.log(
                `Label: ${logName}, angleDeg: ${angleDeg.toFixed(1)}, isLeftSide: ${isLeftSide}, needsFlip: ${needsFlip}, rotationDeg: ${rotationDeg.toFixed(1)}, textAnchor: ${textAnchor}`,
              );
            }
            ...
            if (category.debug || category.name === "rivalrousResources") {
              console.log(
                `Creating sub-wedge for item: "${logName}" in category: ${category.name}`,
              );
            }
        ...
        wedge
          .on("mouseenter", function (event) {
            ...
            // FIX #2: Add debug logging to confirm hover events are firing
            console.log(`Hover ENTER on wedge for category: ${category.name}`);
          ...
          .on("mouseleave", function (event) {
            ...
            // FIX #2: Add debug logging to confirm hover events are firing
            console.log(`Hover LEAVE on wedge for category: ${category.name}`);
  ...
  try {
    ...
    switch (icon.toLowerCase()) {
      ...
      default:
        ...
        const boxLine = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "polyline",
        );
        ...
        const boxLine2 = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "line",
        );
...
function augmentCardValues(card: Card): string[] {
  ...
  const existingValues = card.values
    ? Object.keys(card.values).map((key) => {
        // Strip the 'value_' prefix if present and convert to readable format
        return key.startsWith("value_")
          ? key.substring(6).replace(/-/g, " ")
          : key;
      })
    : [];
...
function augmentCardCapabilities(card: Card): string[] {
  ...
  const existingCapabilities = card.capabilities
    ? Object.keys(card.capabilities).map((key) => {
        // Strip the 'capability_' prefix if present and convert to readable format
        return key.startsWith("capability_")
          ? key.substring(11).replace(/-/g, " ")
          : key;
      })
    : [];
  ...
  const additionalCapability =
    additionalCapabilities[cardIdSum % additionalCapabilities.length];
...
export function createLinks(
  ...
  agreements.forEach((agreement) => {
    ...
    console.log(
      `Found ${participatingCardIds.length} participating cards for agreement ${agreementId}:`,
    ...
    if (participatingCardIds.length < 2) {
      console.warn(
        `Agreement ${agreementId} has fewer than 2 cards, skipping link creation`,
      );
      return;
    }
    ...
    if (agreement.obligations && agreement.obligations.length > 0) {
      ...
      const obligationActorIds = [
        ...new Set(agreement.obligations.map((o) => o.fromActorId)),
      ];
    ...
    console.log(
      `Creating PRIMARY obligation link: ${creatorCardId} → ${agreementId}`,
    );
    links.push({
    ...
    const otherCardIds = participatingCardIds.filter(
      (cardId) => cardId !== creatorCardId,
    );
  ...
  try {
    ...
    if (externalActorCardMap && externalActorCardMap.size > 0) {
      ...
      console.log("d3GraphUtils: Using external actor-card map:", {
        size: externalActorCardMap.size,
        entries: Array.from(externalActorCardMap.entries()),
      });
      ...
      cards.forEach((card) => {
        if (card.actor_id) {
          ...
          console.log(
            `d3GraphUtils: Mapping actor ${card.actor_id} to card ${card.card_id}`,
          );
        }
      });
      ...
      if (
        ...
        console.log(
          "d3GraphUtils: Creating synthetic actor-card mappings as fallback",
        );
        ...
        cards.forEach((card) => {
          ...
          console.log(
            `d3GraphUtils: Created synthetic mapping: ${syntheticActorId} -> ${card.card_id}`,
          );
        });
      }
    }
    ...
    const linkElements = linkGroup
      ...
      .attr("stroke-dasharray", (d) =>
        d.type === "obligation" ? "none" : "4,2",
      ) // Dashed lines for benefits
    ...
    const nodeElements = nodeGroup
      ...
      .attr(
        "class",
        (d) => `node node-${d.type} ${d.id === activeCardId ? "active" : ""}`,
      )
      .attr("id", (d) => `node-${d.id}`)
      .on("click", (event, d) => {
    ...
    nodeElements
      ...
      .attr("class", (d) =>
        d.type === "actor" ? "actor-circle" : "agreement-circle",
      );
    ...
    nodeLabels.each(function (this: SVGGElement, d: D3Node) {
      ...
      if (d.type === "actor") {
        ...
        const textWidth =
          (tempText.node() as SVGTextElement)?.getComputedTextLength() ||
