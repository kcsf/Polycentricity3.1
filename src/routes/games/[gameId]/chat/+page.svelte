<script lang="ts">
        ...
        function switchToPrivateChat(userId: string) {
                // Skip if trying to chat with yourself
                if ($userStore.user && userId === $userStore.user.user_id) return;
                
                // For development - this shouldn't happen in normal operation
                if (!$userStore.user) {
                    console.warn('No user logged in. Using development mode for private chat.');
                }
        ...
        function isCurrentUser(userId: string): boolean {
                ...
                // For development: use a fake user ID when user is not logged in
                return userId === 'dev-user-' + Date.now();
        }
</script>
