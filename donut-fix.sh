#!/bin/bash
# Create a temporary file with our new implementation
cat > new-donut-rings.ts << 'EOF'
...
# Now find the start and end lines
START_LINE=$(grep -n "^export function addDonutRings" src/lib/utils/d3GraphUtils.ts | cut -d: -f1)
END_LINE=$(tail -n +${START_LINE} src/lib/utils/d3GraphUtils.ts | grep -n "^}" | head -n1 | cut -d: -f1)
END_LINE=$((START_LINE + END_LINE - 1))

# Now replace those lines in the original file
sed -i "${START_LINE},${END_LINE}d" src/lib/utils/d3GraphUtils.ts
sed -i "${START_LINE}i\\$(cat new-donut-rings.ts)" src/lib/utils/d3GraphUtils.ts

echo "Fixed addDonutRings function from line ${START_LINE} to ${END_LINE}"
